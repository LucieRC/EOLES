...
"""OUTPUTS
    There are 5 output files :
        ...
        - Hourly-Deficit    : A temporary file for the nuc opti module, with residual demand and installed nuclear capacity
...
# Hourly deficit / residual demand
...
try:
    with open(hourly_file,"w",newline="") as hourly:
        ...
        title_row = ["hour", "deficit"]
        title_row += ["nuc_capa"]
        hourly_writer.writerow(title_row)
        for h in model.h:
            hourly_data = [h]
            deficit=demand[h]-sum(model.gene[vre, h] for vre in model.vre)
            hourly_data.append(round(pyo.value(deficit),2))
            if h==0: hourly_data.append(round(pyo.value(model.capa['nuc']),2))
